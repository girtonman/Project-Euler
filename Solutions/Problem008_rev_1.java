//skips 0 blocks
//target answer  = 23514624000
//new loop count = 1974
//old loop count = 14818
public class Problem008_rev_1 {
	private static long loopCounter = 0;
	public static void main(String[] args) {
		int run = 13;
		boolean roll = true;
		long rollingProduct = 0;
		long largestProduct = 0;
		int largestProductIndex = 0;
		String numbers = "";
		char[] numbersAsCharArray = numbers.toCharArray();
		int[] numbersAsIntArray = new int[numbersAsCharArray.length];
		//convert to ints
		for (int i = 0; i < numbersAsCharArray.length; i++) {
			numbersAsIntArray[i] = Integer.parseInt("" + numbersAsCharArray[i]);
			loopCounter++;
		}
		//calculate answer
		for (int i = 0; i < numbersAsIntArray.length - run; i++) {
			if(roll) {
				rollingProduct = calculateRunProd(i, run, numbersAsIntArray);
				if(rollingProduct == 0) {
					i = i + (run - 1);
					continue;
				}
				i = i + (run - 1);
				roll = false;
				continue;
			}
			//watch out for incoming zeroes
			if(numbersAsIntArray[i + (run - 1)] == 0) {
				roll = true;
				rollingProduct = 0;
				continue;
			}
			if(numbersAsIntArray[i] == 0) {
				roll = true;
				rollingProduct = 0;
				i++;
				continue;
			}
			rollingProduct = rollingProduct * numbersAsIntArray[i] / numbersAsIntArray[i - run];
			if (rollingProduct > largestProduct) {
				largestProduct = rollingProduct;
				largestProductIndex = i;
				System.out.println("Largest current product tail is at " + i + " with numbers multiplying to: " + largestProduct);
			}
			loopCounter++;
		}
		
		//answers and stats
		System.out.println("Numbers in set (in reverse): ");
		for (int i = largestProductIndex; i > largestProductIndex - run; i--) {
			System.out.print(numbersAsIntArray[i] + ", ");
		}
		System.out.println("Total loops: " + loopCounter);
	}

	// returns product of x numbers where x is the run, starting at the index
	// provided
	public static long calculateRunProd(int index, int run, int[] numbers) {
		long product = 1;
		for (int i = index; i < index + run; i++) {
			product = product * numbers[i];
			loopCounter++;
		}
		return product;
	}
}
